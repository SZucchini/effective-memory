...
def getbone_unified(seq, boneindex):
    bs, ss = seq.size(0), seq.size(1)
    seq = seq.view(-1, seq.size(2), seq.size(3))
    
    bone_indices = torch.tensor(boneindex, device=seq.device, dtype=torch.long)
    bone = seq[:, bone_indices[:, 1]] - seq[:, bone_indices[:, 0]]
    bone = bone.transpose(0, 1)
    
    bone_length_2d = torch.pow(torch.pow(bone, 2).sum(2), 0.5)
    bone_length = bone_length_2d.transpose(0, 1).view(bs, ss, -1, 1)
    
    bonesum = bone_length_2d.transpose(0, 1).unsqueeze(2)
    bonedirect_flat = bone.transpose(0, 1) / bonesum
    bonedirect = bonedirect_flat.view(bs, ss, -1, 3)
    
    return bone_length, bonedirect

def getbonelength(seq, boneindex):
    bone_length, _ = getbone_unified(seq, boneindex)
    return bone_length

def getbonedirect(seq, boneindex):
    _, bonedirect = getbone_unified(seq, boneindex)
    return bonedirect
...
class DDHPose(nn.Module):
    ...
    def model_predictions_dir_bone(self, x_dir, x_bone, inputs_2d, input_2d_flip, t):
        ...
        # input 2d flip (only during inference)
        ...
        if not self.is_train and pred_pose_flip is not None:
            pred_pose_flip[:, :, :, :, 0] *= -1
    ...
    def prepare_targets(self, targets):
        bone_length, dir = getbone_unified(targets, boneindex)
        
        batch_size, seq_len, n_joints = targets.shape[:3]
        device = targets.device
        
        targets_dir = torch.zeros(batch_size, seq_len, n_joints, 3, device=device)
        targets_bone_length = torch.zeros(batch_size, seq_len, n_joints, 1, device=device)
